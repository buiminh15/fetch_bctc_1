...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('http://www.sfn.vn/?vnTRUST=mod:news|act:category|catID:2', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.SFN);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.SFN);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của SFN ::: ${name}`);
        })
      );
