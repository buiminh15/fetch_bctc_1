...
async function fetchAndExtractData() {
  try {
    const response = await axios.get(
      'https://ezir.fpts.com.vn/ThongTinDoanhNghiep/GetTinDoanhNghiepCongBo?stock_code=HGM&lang=vi-VN',
      {
        headers: {
    ...
    const newNames = await filterNewNames(names, COMPANIES.HGM);
    console.log('📢 [bctc-geg.js:44]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.HGM);

      // Gửi thông báo Telegram cho từng báo cáo mới;
      await Promise.all(
        newNames.map(name =>
          sendTelegramNotification(`Báo cáo tài chính của HGM::: ${name}`)
        )
      );
