...
async function fetchAndExtractData() {
  ...
  try {
    const response = await axios.get(`https://www.pharmedic.com.vn/vn/thong-bao-co-dong-nam-${currentYear}.html`, {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.PMC);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.PMC);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của PMC ::: ${name}`);
        })
      );
