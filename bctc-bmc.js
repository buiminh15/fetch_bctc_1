...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('http://bimico.vn/VanBan/Nhom_VanBan/16', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.BMC);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.BMC);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của BMC ::: ${name}`);
        })
      );
