...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://tamduchearthospital.com/gioi-thieu/cong-bo-thong-tin/', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.TTD);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.TTD);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của TTD ::: ${name}`);
        })
      );
