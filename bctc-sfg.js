...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://phanbonmiennam.com.vn/co-dong/thong-tin-tai-chinh-sfg/', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.SFG);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.SFG);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của SFG ::: ${name}`);
        })
      );
