...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('http://ssc.com.vn/vi/quan-he-nha-dau-tu/bao-cao-tai-chinh/', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.SSC);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.SSC);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của SSC ::: ${name}`);
        })
      );
