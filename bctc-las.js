...
async function fetchAndExtractData() {
  try {
    ...
    for (const element of PAGES) {
      console.log('📢 [bctc-las.js:24]', element);
      const response = await axios.get(`https://supelamthao.vn/chuyen-muc-co-dong/49-thong-tin-co-dong?page=${element}`, {
        headers: {
          'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.LAS);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.LAS);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của LAS ::: ${name}`);
        })
      );
