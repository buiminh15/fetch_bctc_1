...
async function fetchAndExtractData() {
  try {
    const response = await axios.get(
      'https://ezir.fpts.com.vn/ThongTinDoanhNghiep/GetTinDoanhNghiepCongBo?stock_code=SZE&lang=vi-VN',
      {
        headers: {
    ...
    const newNames = await filterNewNames(names, COMPANIES.SZE);
    console.log('📢 [bctc-geg.js:44]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.SZE);

      // Gửi thông báo Telegram cho từng báo cáo mới;
      await Promise.all(
        newNames.map(name =>
          sendTelegramNotification(`Báo cáo tài chính của SZE::: ${name}`)
        )
      );
