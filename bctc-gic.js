...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://greenicd.com.vn/vn/bao-cao-tai-chinh.html', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.GIC);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.GIC);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của GIC ::: ${name}`);
        })
      );
