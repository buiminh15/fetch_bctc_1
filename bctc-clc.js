...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('http://www.catloi.com.vn/index.php?option=com_content&task=blogcategory&id=3&Itemid=12', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.CLC);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.CLC);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của CLC ::: ${name}`);
        })
      );
